...
class Tags extends React.Component {
  ...
  addTag = (tagToAdd, count, tags, updateState) => {
    let tempSampleArray = tags.tagsSample;
    tempSampleArray.splice(count, 1);
    let tempTagsArray = tags.tagsArray;
    tempTagsArray.push(tagToAdd);

    let tempObject = {
      tag: tags.tag,
      tagsArray: tempTagsArray,
      tagsSample: tempSampleArray,
    };
    updateState(tempObject);
  };
    ...
    return (
      <View style={StyleSheet.flatten([styles.container, containerStyle])}>
        ...
        <View style={StyleSheet.flatten([styles.tagsView, tagsViewStyle])}>
          {tags.tagsSample.map((item, count) => {
            return (
              <View
                ...
                <TouchableOpacity
                  onPressIn={() => this.addTag(item, count, tags, updateState)}>
                  {addElement ? (
                    addElement
                  ) : (
                    <Image
